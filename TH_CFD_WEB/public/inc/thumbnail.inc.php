...
class Thumbnail {
    ...
    private function calcImageSize($width,$height) {
        ...
        if($this->maxWidth > 0 && $width <= $height) {
...
//            }
        ...
        if($this->maxHeight > 0 && $width > $height) {
...
//            }
	 ...
	 * @param int $cropSize
	 */
	public function cropFromCenter($cropSizeW, $cropSizeH) {
	    if($cropSizeW > $this->currentDimensions['width']) $cropSizeW = $this->currentDimensions['width'];
	    if($cropSizeH > $this->currentDimensions['height']) $cropSizeH = $this->currentDimensions['height'];

	    $cropX = intval(($this->currentDimensions['width'] - $cropSizeW) / 2);
		if($this->currentDimensions['width']>$this->currentDimensions['height'])
			$cropY = intval(($this->currentDimensions['height'] - $cropSizeH) / 2);
		else
			$cropY = intval(($this->currentDimensions['height'] - $cropSizeH) / 4);

	    if(function_exists("ImageCreateTrueColor")) {
			$this->workingImage = ImageCreateTrueColor($cropSizeW,$cropSizeH);
		}
		else {
			$this->workingImage = ImageCreate($cropSizeW,$cropSizeH);
		}
		...
		$this->oldImage = $this->workingImage;
		$this->newImage = $this->workingImage;
		$this->currentDimensions['width'] = $cropSizeW;
		$this->currentDimensions['height'] = $cropSizeH;
	}
