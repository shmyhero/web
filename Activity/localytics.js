...
var localyticsReady = false;
function waitDone(callback){
                setTimeout(function(){
                        console.log(localyticsReady);
                        if(localyticsReady){
                                callback();
                                localyticsReady = false;
                        }
else{
	waitDone(callback);
}
...
function ll(event, callback){
	var localyticsSession = LocalyticsSession("ccb5dabdce128f25701a415-f3c453de-796b-11e4-a823-009c5fda0a25",{logger:true,namespace:'trade',callback:function(){localyticsReady = true;}});
