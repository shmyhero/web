...
function showsecond0(x){
	var URL = "stores_AjaxSmallClass.php?value="+x;
	Request.open("GET",URL,true);
	Request.onreadystatechange = Show_Second0;
	Request.send(null);
}

function Show_Second0(){
	if(Request.readyState==4 &&Request.status==200){
	//显示二级框
	document.getElementById("smallclass0").style.display ='';
	//读取返回的xml数据
	var myData=Request.responseXML.getElementsByTagName("row");
	//如果数据长度为0，则表示没有取到数据，则不显示二级菜单
	if(myData.length == 0){
		document.getElementById("smallclass0").style.display = 'none';   
	}
	//alert(myData.length);
	var myStr=new Array();
	...
	for(var i = 0;i<myData.length;i++){
		...
		document.getElementById("smallclass0").options.length =0;   
		for(var j=0;j<myStr.length;j++){
			document.getElementById("smallclass0").options[document.getElementById("smallclass0").options.length] =new Option(myStr[j],myValue[j]);
		}
	}
